function dataBase(content, element, x , y, z) {
    ...
    var img = x.find('img');
    if(img.length) {
        var name = img.first().attr('src').split('/'); //! Ostrzeżenie: Due to both security and performance concerns, this may not be set using dynamic values which have not been adequately sanitized. content can lead to security issues or fairly serious performance degradation.
    ...
    if(element['options']['wyswietlajnastronie'] == '1' || element['options']['mobilnesterowanie'] == '1') {
        z.append(`<div style="font-weight: bold;">` + answer + '</div>');
    }
    ...
    if(element['options']['zaznaczajodpowiedzi'] == '1') {
        if(answer != 'Brak danych :(') {
            ...
            $(content).find('.answer').first().children().each(function(i, k) {
